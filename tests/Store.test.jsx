...
beforeEach(() => { // maybe remove beforeEach,
    vi.resetAllMocks();
...
describe("add to cart button", () => {
    ...
    it("does not work when letter is set as itemQuantity", async () => {
        const StoreItem = await import("../src/components/utilities/StoreItem/StoreItem.jsx");

        StoreItem.default = vi.fn(({ addToCartHandler }) => {
            const item = {
                id: 2,
                title: "Mens Casual Premium Slim Fit T-Shirts",
                price: 22.3,
            ...
            return (
                <>
                    <button onClick={() => addToCartHandler(item, "e")}>Add to cart</button>
                </>
            )
        })
        const MockParent = ({ initialMockCart }) => {
        ...
        const removeFromCartButtons = screen.queryAllByLabelText(/remove item from cart/i);

        expect(removeFromCartButtons).toHaveLength(0);
